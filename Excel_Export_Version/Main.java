...
public class Main {
    ...
    public static void main(String [] args)
    {
        ...
        double[][][][] results = CutsomSortComparison_Excel.compareSorts();

        String[] headers = {"Thread Count", "Custom Sort", "Collections.sort"};
        String[][] data = new String[CutsomSortComparison_Excel.threadCounts.length + 1][headers.length];
        data[0] = headers;
        
        for (int type=0; type<CutsomSortComparison_Excel.arrTypes.length; type++) {
            for (int n=0; n<CutsomSortComparison_Excel.arrLengths.length; n++) {
                for (int threads=0; threads<CutsomSortComparison_Excel.threadCounts.length; threads++) {
                    data[threads+1][0] = Integer.toString(CutsomSortComparison_Excel.threadCounts[threads]);
                    data[threads+1][1] = Double.toString(results[0][type][n][threads]);
                    data[threads+1][2] = Double.toString(results[1][type][n][threads]);
                }
                File typeDir = new File("Data/" + CutsomSortComparison_Excel.arrTypes[type].getSimpleName());
                if (!typeDir.exists())
                    typeDir.mkdir();

                String fileName = "./Data/" +
                                CutsomSortComparison_Excel.arrTypes[type].getSimpleName() + "/" +
                                CutsomSortComparison_Excel.arrLengths[n] + "_elements";
